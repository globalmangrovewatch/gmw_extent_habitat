...
for index, tile in gmw_tiles_gdf.iterrows():
    ...
    if tile["MinY"] < 0:
        abs_lat_int = rsgislib.tools.utils.zero_pad_num_str(math.fabs(tile["MinY"]), str_len=2, integerise=True)
    ...
    if tile["MinX"] < 0:
        abs_lon_int = rsgislib.tools.utils.zero_pad_num_str(math.fabs(tile["MinX"]), str_len=3, integerise=True)
